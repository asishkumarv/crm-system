...
export default function AdminLogin() {
  ...
  const login = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Authentication failed. Please check your admin credentials.");
      setVisible(true);
    } finally {
      setLoading(false);
    }
          ...
          >
            <View style={[styles.glassWrapper, { maxWidth: isDesktop ? 450 : isTablet ? 400 : '100%' }]}>
              <View style={styles.logoSection}>
                ...
                <Text variant="bodyMedium" style={styles.brandTagline}>ADMINISTRATOR PORTAL</Text>
              </View>

              <Card style={styles.card} mode="elevated">
                <Card.Content style={styles.cardContent}>
                  <Text variant="titleLarge" style={styles.sectionTitle}>Login to Dashboard</Text>
                  ...
                  <View style={styles.footer}>
                    ...
                    <TouchableOpacity onPress={() => router.replace("/employeeLogin")} style={{ marginTop: 16 }}>
                      <Text style={[styles.link, { color: "#00796B" }]}>Switch to Employee Login</Text>
                    </TouchableOpacity>
                  </View>
                </Card.Content>
              </Card>
            ...
            <Portal>
              <Dialog visible={visible} onDismiss={() => setVisible(false)} style={styles.dialog}>
                <Dialog.Title style={styles.errorTitle}>Security Notice</Dialog.Title>
                <Dialog.Content>
                  <Text variant="bodyMedium" style={styles.errorMsg}>{error}</Text>
                </Dialog.Content>
                <Dialog.Actions>
                  <Button onPress={() => setVisible(false)} textColor="#1565C0">RETRY</Button>
                </Dialog.Actions>
              </Dialog>
            </Portal>
          </ScrollView>
