...
interface AdminProfile { name: string; email: string; phone?: string; company?: string; createdAt?: string; }
...
export default function AdminProfilePage() {
  ...
  const getInitials = (name: string) => name ? name.split(" ").map(n => n[0]).join("").substring(0, 2).toUpperCase() : "AD";
  ...
  return (
    <View style={styles.outerContainer}>
      <Appbar.Header style={styles.appbar} elevated>
        <Appbar.BackAction onPress={() => router.back()} color="#1A237E" />
        <Appbar.Content title="My Profile" titleStyle={styles.appbarTitle} />
        {!editMode
          ? <Appbar.Action icon="pencil-outline" onPress={() => setEditMode(true)} color="#1A237E" />
        ...
        <View style={styles.wrapper}>
          ...
          <Surface style={styles.heroBanner} elevation={2}>
            ...
            <View style={styles.bannerContent}>
              ...
              <View style={styles.heroInfo}>
                <Text variant="headlineSmall" style={styles.heroName}>{profile.name || "Admin"}</Text>
                <Text variant="bodyMedium" style={styles.heroEmail}>{profile.email}</Text>
                <Chip icon="shield-crown" style={styles.roleChip} textStyle={styles.roleChipText}>Administrator</Chip>
              </View>
            </View>
          </Surface>
          ...
          <View style={styles.statsRow}>
            <Surface style={styles.statBox} elevation={1}>
              ...
              <Text style={styles.statValue}>{profile.company || "—"}</Text>
          ...
          <Card style={styles.card} mode="elevated">
            <Card.Content>
              ...
              <TextInput label="Full Name" value={profile.name} onChangeText={t => setProfile({ ...profile, name: t })} mode="outlined" editable={editMode} left={<TextInput.Icon icon="account-outline" />} style={styles.fieldInput} outlineColor={editMode ? "#1565C0" : "#E2E8F0"} activeOutlineColor="#1A237E" textColor="#000" />
              ...
              <TextInput label="Phone Number" value={profile.phone || ""} onChangeText={t => setProfile({ ...profile, phone: t })} mode="outlined" editable={editMode} keyboardType="phone-pad" left={<TextInput.Icon icon="phone-outline" />} style={styles.fieldInput} outlineColor={editMode ? "#1565C0" : "#E2E8F0"} activeOutlineColor="#1A237E" textColor="#000" />
              <TextInput label="Company / Organization" value={profile.company || ""} onChangeText={t => setProfile({ ...profile, company: t })} mode="outlined" editable={editMode} left={<TextInput.Icon icon="office-building-outline" />} style={styles.fieldInput} outlineColor={editMode ? "#1565C0" : "#E2E8F0"} activeOutlineColor="#1A237E" textColor="#000" />
              {editMode && (
                <Button mode="contained" onPress={handleSave} loading={saving} disabled={saving} style={styles.saveBtn} buttonColor="#1A237E" textColor="white" labelStyle={{ fontWeight: "700" }} icon="content-save-outline">Save Changes</Button>
              )}
            </Card.Content>
          </Card>
            ...
            <Card.Content>
              ...
              <TouchableOpacity style={styles.securityRow} onPress={() => setChangePwVisible(true)}>
                <View style={styles.securityRowLeft}>
                  <IconButton icon="lock-outline" iconColor="#1A237E" size={22} style={styles.secIcon} />
                  <View>
      ...
      <Portal>
        <Dialog visible={changePwVisible} onDismiss={() => setChangePwVisible(false)} style={styles.dialog}>
          ...
          <Dialog.Actions style={styles.dialogActions}>
            ...
            <Button mode="contained" onPress={handleChangePassword} loading={pwLoading} disabled={pwLoading} buttonColor="#1A237E" textColor="white" style={[styles.dialogBtn, { flex: 1 }]}>Update</Button>
          </Dialog.Actions>
        </Dialog>
        ...
        <Dialog visible={dialogVisible} onDismiss={() => setDialogVisible(false)} style={styles.dialog}>
          ...
          <Dialog.Actions><Button onPress={() => setDialogVisible(false)} textColor="#1565C0">OK</Button></Dialog.Actions>
        </Dialog>
      </Portal>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  appbarTitle: { fontWeight: "900", color: "#1A237E", letterSpacing: 1 },
  loadingContainer: { flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "#fff" },
  loadingText: { marginTop: 16, fontWeight: "600", color: "#1A237E" },
  heroBanner: { borderRadius: 24, overflow: "hidden", backgroundColor: "#fff", borderWidth: 1, borderColor: "#E2E8F0" },
  bannerBg: { height: 80, backgroundColor: "#1A237E", position: "absolute", top: 0, left: 0, right: 0 },
  bannerContent: { flexDirection: "row", alignItems: "flex-end", padding: 20, paddingTop: 44, gap: 16 },
  avatar: { backgroundColor: "#3949AB", borderWidth: 4, borderColor: "#fff" },
  heroInfo: { flex: 1, paddingBottom: 4 },
  heroName: { fontWeight: "900", color: "#1E293B" },
  heroEmail: { color: "#64748B", marginTop: 2 },
  roleChip: { alignSelf: "flex-start", marginTop: 8, backgroundColor: "#EEF2FF" },
  roleChipText: { color: "#3730A3", fontWeight: "700", fontSize: 12 },
  ...
  secIcon: { backgroundColor: "#EEF2FF", margin: 0 },
  ...
  dialogTitle: { fontWeight: "800", color: "#1A237E" },
